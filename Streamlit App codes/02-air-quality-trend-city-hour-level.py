...
if (date_option is not None):
    ...
    pd_df = pd.DataFrame(
        sf_df,
        columns=['Hour', 'PM25', 'PM10', 'SO2', 'NO2', 'NH3', 'CO', 'O3']
    )
    
    # Display Hourly Trend Charts
    st.bar_chart(pd_df.set_index('Hour'))
    st.divider()
    st.line_chart(pd_df.set_index('Hour'))

    # Create a Pie Chart for pollutant proportions
    hourly_pollutant_sum = pd_df.drop('Hour', axis=1).sum()

    pie_fig = px.pie(
        names=hourly_pollutant_sum.index,  # Pollutant names
        values=hourly_pollutant_sum.values,  # Pollutant values
        title=f"Proportion of Pollutants for {city_option} on {date_option}",
        hole=0.4  # Donut Pie Chart
    )
